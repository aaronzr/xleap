...
import sparklines_v2.archive.fetch as fetch_module
import sparklines_v2.archive.hierarchy as hierarchy_module
...
"""
    ...
    def test_uses_explicit_monitor_specs(self):
        ...
        with mock.patch.object(hierarchy_module, "fetch_archive_batch", side_effect=_fake_fetch):
    ...
    def test_fetch_archive_batch_caps_worker_count(self):
        from sparklines_v2.beamlines import ARCHIVE_MAX_WORKERS

        pv_names = [f"PV:{idx}" for idx in range(ARCHIVE_MAX_WORKERS + 5)]
        captured = {}
        ...
        with (
            mock.patch.object(fetch_module, "ThreadPoolExecutor", _DummyExecutor),
            mock.patch.object(fetch_module, "as_completed", return_value=[]),
            mock.patch.object(fetch_module, "tqdm", return_value=_DummyProgress()),
        ):
            fetch_module.fetch_archive_batch(
                pv_names,
                dt.datetime(2026, 1, 1, 0, 0, 0),
                dt.datetime(2026, 1, 1, 1, 0, 0),
            )

        self.assertEqual(captured["max_workers"], ARCHIVE_MAX_WORKERS)
