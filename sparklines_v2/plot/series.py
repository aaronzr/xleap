"""Archive-timeseries plotting utilities.

Fetches a single PV from the archiver, plots one or more series with axis
formatting helpers, computes trailing rolling mean/std, plots tuning-period
overlays and per-group tuning bars, and provides the vector-distance helper
used by the overlay alpha weighting.
"""
...
from sparklines_v2.archive.fetch import format_archive_time
...
from sparklines_v2.plot.tuning import (
    _event_times_from_value_changes,
    detect_tuning,
)
...
def get_archive_data(
    ...
    *,
    ...
    response = requests.get(
        ...
        params={
            'pv': pv,
            'from': format_archive_time(from_time, local_timezone=local_timezone),
            'to': format_archive_time(to_time, local_timezone=local_timezone),
        },
        timeout=timeout,
    )
