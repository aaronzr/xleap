...
DEFAULT_SNAPSHOT_DELTA = timedelta(seconds=3)
...
def fetch_archive_snapshot(
    ...
    *,
    ...
    pv_names = [GAMMA_PV, *(kact_pvs or list_kact_pvs())]
...
def add_archive_snapshot(
    ...
    *,
    ...
    with connect(db_path) as connection:
        upsert_snapshot_rows(connection, rows, update_wide=True)
    return rows
...
def backfill_incomplete_snapshots(
    *,
    ...
    def fetch_one(nominal_time: dt) -> list[SnapshotRow]:
        return fetch_archive_snapshot(
            nominal_time,
            snapshot_delta=snapshot_delta,
            kact_pvs=kact_pvs,
            archive_timeout_seconds=archive_timeout_seconds,
